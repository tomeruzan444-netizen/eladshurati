...
export const blockInserts = [
  ...
  /* /ייעוץ-עסקי-לאדריכלים/ - "ייעוץ עסקי לאדריכלי נוף", requested by Tomer
   * (15.9.2026). Nothing on the site mentions landscape architecture, so the
   * section covers what is specific to it as a business rather than restating
   * the page's general advice for architects. */
  {
    id: 'architects-landscape',
    page: '/ייעוץ-עסקי-לאדריכלים/',
    after: /^מהניסיון שלי, שיווק נכון הוא המפתח להצלחה/,
    why: 'הביטוי "ייעוץ עסקי לאדריכלי נוף", בזווית שהעמוד לא מכסה: מי מביא את העבודה, תשלום שתלוי בפרויקט של מישהו אחר, פיקוח שלא מתומחר, ותיק עבודות שנראה הכי טוב רק אחרי שהצמחייה גדלה',
    blocks: [
      { type: 'heading', level: 2, text: 'ייעוץ עסקי לאדריכלי נוף: כשהעבודה מגיעה בסוף הפרויקט' },
      {
        type: 'richtext',
        html: '<p>אדריכלות נוף נשמעת כמו עוד תחום בתוך עולם האדריכלות, אבל מבחינה עסקית היא עובדת אחרת. אדריכל נוף מגיע לא פעם דרך אדריכל הבניין או היזם, ולא ישירות מהלקוח. לוחות הזמנים שלו תלויים בפרויקט שהוא לא מנהל, והביצוע בשטח מגיע בשלב האחרון - בדיוק כשהתקציב של כולם כבר מתוח.</p><p>וזה משנה את השאלות העסקיות שכדאי לשאול:</p>',
      },
      {
        type: 'list',
        ordered: false,
        items: [
          '<strong>מי מביא את העבודה?</strong> אם רוב הפרויקטים מגיעים משניים או שלושה אדריכלים ויזמים, העסק תלוי בהם יותר ממה שנראה. כדאי לבנות קשר ישיר גם עם <a href="/ייעוץ-עסקי-לקבלנים/">קבלנים</a> ויזמים, ולא רק דרך מי שהזמין את התכנון.',
          '<strong>על מה משולם כל שלב?</strong> כשהתשלום צמוד לסיום הבנייה, התזרים של המשרד נקבע לפי הקצב של מישהו אחר. עדיף לקשור את שכר הטרחה לאבני הדרך של התכנון עצמו - סקיצה, תכנית מפורטת ותכניות לביצוע.',
          '<strong>האם הפיקוח מתומחר?</strong> ביקורים בשטח בזמן הביצוע הם חלק מהאחריות המקצועית. כשהם לא מוגדרים בהצעת המחיר, הם הופכים לשעות עבודה בחינם.',
        ],
      },
      {
        type: 'richtext',
        html: '<p>יש גם אתגר שיווקי שייחודי לתחום: גינה נראית במיטבה רק אחרי שהצמחייה גדלה, לפעמים שנה או שנתיים אחרי המסירה. מי שמצלם את הפרויקט ביום המסירה מציג את העבודה שלו בגרסה החלשה ביותר שלה. כדאי לתכנן מראש צילום חוזר לפרויקטים נבחרים, ולבקש מהלקוח אישור לחזור לשטח כבר כשחותמים על ההסכם.</p>',
      },
    ],
  },
]
